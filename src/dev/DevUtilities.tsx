...
export function DevUtilities({
  ...
  const [activeTab, setActiveTab] = useState<'state' | 'props' | 'performance' | 'controls'>('state');
  ...
  const filteredState = useMemo(() => {
    ...
    return filtered;
  }, [scenarioState, searchQuery, filterType]);
  ...
  const filteredProps = useMemo(() => {
    ...
    return filtered;
  }, [mergedProps, currentScenario?.props, searchQuery]);
  ...
  const handleCopyData = useCallback(async () => {
    const data = activeTab === 'state' ? filteredState : 
                 activeTab === 'controls' ? mergedProps :
                 filteredProps;
    try {
  ...
  return (
    <div className={`fixed ${positionClasses[position]} z-50 w-96 max-w-[90vw] max-h-[80vh] overflow-hidden`}>
              ...
              <div>
                ...
                {Object.keys(filteredState).length === 0 ? (
                  <div className="text-center py-8 text-gray-500 text-sm">
                    {searchQuery || filterType !== 'all' ? 'No matching state found' : 'No state data available'}
                  </div>
                ) : (
                          ...
                          <div key={key} className="p-2 border rounded bg-gray-50">
                            ...
                            <div className="text-sm">
                              <span className={`font-mono ${
                                propData.type === 'string' ? 'text-green-600' :
                                propData.type === 'number' ? 'text-orange-600' :
                                propData.type === 'boolean' ? 'text-purple-600' :
                                'text-gray-600'
                              }`}>
                                {propData.type === 'string' ? `"${propData.value}"` : String(propData.value)}
                              </span>
                              {propData.defaultValue !== undefined && propData.defaultValue !== propData.value && (
                                <div className="text-xs text-gray-500 mt-1">
                                  Default: {propData.type === 'string' ? `"${propData.defaultValue}"` : String(propData.defaultValue)}
                                </div>
                              )}
                            </div>
                          </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="bg-gray-100 rounded p-3">
                    <div className="text-xs text-gray-600">
                      Heap Size: {((performance as any).memory?.usedJSHeapSize / 1024 / 1024 || 0).toFixed(2)} MB
                    </div>
                    <div className="text-xs text-gray-600">
                      Total Heap: {((performance as any).memory?.totalJSHeapSize / 1024 / 1024 || 0).toFixed(2)} MB
                    </div>
                    <div className="text-xs text-gray-600">
                      Heap Limit: {((performance as any).memory?.jsHeapSizeLimit / 1024 / 1024 || 0).toFixed(2)} MB
                    </div>
                  </div>
                </div>
              </div>
