...
const ControlInput = React.memo(
  ({ propName, control, value, onChange, validation, disabled = false }: ControlInputProps) => {
    ...
    switch (control.type) {
      ...
      case 'url':
        return (
          <div className='space-y-1'>
            ...
            />
            {hasError && (
              <div className='text-xs text-destructive'>{validation.errors.join(', ')}</div>
            )}
          </div>
        );
      ...
      case 'range': {
        ...
        const fallbackMin = Number.isFinite(Number(control.min)) ? (control.min as number) : 0;
        const numValue = hasNumeric ? (parsed as number) : fallbackMin;
        return (
...
export const PropControls: React.FC<PropControlsProps> = ({
  ...
  const filteredControls = useMemo(() => {
    ...
    if (!term) return controls; // Preserve original reference when no filtering

    const filtered: ControlsConfig = {};
    Object.entries(controls).forEach(([propName, control]) => {
      if (
        propName.toLowerCase().includes(term.toLowerCase()) ||
        control.label?.toLowerCase().includes(term.toLowerCase()) ||
        control.description?.toLowerCase().includes(term.toLowerCase())
      ) {
  ...
  const handlePropChange = useCallback(
    async (propName: string, newValue: any) => {
      // Increment validation token to track the latest change
      const token = ++validationTokenRef.current;
      const oldValue = mergedProps[propName];
      const control = controls[propName];

      if (!control) {
        console.warn(`PropControls: No control found for prop "${propName}"`);
        return;
      }

      // Validate the prop change with error handling
      let validationResult: ValidationResult;
      try {
        validationResult = await validatePropChange(propName, newValue, control);
      } catch (err: any) {
        console.error('Validation error for prop change', { propName, err });
        validationResult = {
          isValid: false,
      ...
      // Ensure only the latest validation result is applied
      if (token !== validationTokenRef.current) {
        return; // Outdated validation; ignore
      }
