...
// -----------------------------
/**
 * Validate that each scenario group has the required shape and log issues.
 * This is a lightweight runtime validator meant to catch structural issues early.
 */
function validateScenarioStructure(definition: Record<string, any>): void {
  try {
    ...
    if (errors.length) {
      // Log all structural errors together for easy discovery.
      errors.forEach(e => console.error(e));
    }
  } catch (e) {
    // Never let validation throw during import; only log.
    console.error('[scenarios] Unexpected error during structure validation:', e);
  }
}

/**
 * Ensure Controls and defaultProps are synchronized where both are present.
 * - Warn if keys mismatch
 * - Warn if default value mismatches
 * - Optionally throw for egregious mismatches (extra defaultProps keys)
 */
  ...
  try {
    Object.entries(definition).forEach(([componentName, group]) => {
      ...
      group.scenarios.forEach((scenario: any) => {
        ...
        // Extra keys in defaultProps not declared in controls: error
        const extraDefaultKeys = defaultKeys.filter(k => !controlKeys.includes(k));
        if (extraDefaultKeys.length) {
          // Throwing helps surface drift during development/build.
          // It’s safe because our scenarios currently keep these in sync.
          throw new Error(
            `[scenarios] ${path}: defaultProps has keys not in controls: ${extraDefaultKeys.join(', ')}`
          );
        }

        // Keys present in controls but missing in defaultProps: warn (not always required)
        ...
        // Compare default values when both exist
          ...
          controlKeys.forEach(key => {
            ...
            if (control && 'defaultValue' in control) {
              ...
              if (typeof controlDefault !== 'undefined') {
                ...
                // Strict equality is sufficient for our primitive defaults
    ...
    // Surface as error; keep it non-fatal for app runtime but fail tests/build if desired
    console.error(e);
    // Intentionally do not re-throw to avoid breaking runtime imports.
  }
}
    ...
    scenarios: [
      {
        ...
        component: () =>
          React.createElement(ChallengeSelection, {
            ...
            isLoading: false,
            error: null,
            onChallengeSelect: (challengeId: string) =>
      ...
      },
      {
        ...
        component: () =>
          React.createElement(ChallengeSelection, {
            ...
            isLoading: false,
            error:
              'Failed to load challenges. Please check your internet connection and try again.',
            onChallengeSelect: (challengeId: string) =>
              console.log('Selected challenge:', challengeId),
          }),
        documentation: {
  ...
  'Card Components': {
    ...
    scenarios: [
      ...
      },
      {
        ...
        component: () =>
          React.createElement('div', { className: 'grid grid-cols-1 gap-4 p-4 max-w-md' }, [
            React.createElement(Card, { key: 'actions' }, [
              React.createElement(CardHeader, { key: 'header' }, [
                React.createElement(
                  CardTitle,
                  { key: 'title' },
                  mockUIComponentData.cardContent.projects[0].title
                ),
                React.createElement(
                  CardDescription,
                  { key: 'desc' },
                  mockUIComponentData.cardContent.projects[0].description
                ),
              ...
              React.createElement(CardContent, { key: 'content' }, [
                React.createElement(
                  'div',
                  { key: 'status', className: 'text-sm text-muted-foreground' },
              ...
              React.createElement(CardFooter, { key: 'footer' }, [
                React.createElement(
                  Button,
                  { key: 'view', variant: 'outline', size: 'sm' },
                  'View Details'
                ),
          ...
          React.createElement('div', { className: 'grid grid-cols-1 gap-4 p-4 max-w-md' }, [
            React.createElement(Card, { key: 'empty' }, [
              ...
              React.createElement(CardContent, { key: 'content' }, [
                React.createElement(
                  'div',
                  { key: 'icon', className: 'flex justify-center text-4xl text-muted-foreground' },
                  '📁'
                ),
              ]),
              React.createElement(CardFooter, { key: 'footer' }, [
        ...
        component: () =>
          React.createElement('div', { className: 'space-y-4 p-4 max-w-md' }, [
            ...
            React.createElement('div', { key: 'error' }, [
              ...
              React.createElement(
                'p',
                { key: 'error-msg', className: 'text-sm text-destructive mt-1' },
        ...
        component: () =>
          React.createElement('div', { className: 'space-y-4 p-4 max-w-md' }, [
            React.createElement('div', { key: 'required' }, [
              ...
              React.createElement(
                'p',
                { key: 'help', className: 'text-sm text-destructive mt-1' },
            ...
            React.createElement('div', { key: 'success' }, [
              ...
              React.createElement(
                'p',
                { key: 'success-msg', className: 'text-sm text-green-600 mt-1' },
  ...
  'Alert Components': {
    ...
    scenarios: [
      ...
      },
      {
        ...
        component: () =>
          React.createElement('div', { className: 'space-y-4 p-4' }, [
          ...
          React.createElement('div', { className: 'space-y-4 p-4' }, [
            ...
            React.createElement(Alert, { key: 'long', variant: 'warning' }, [
              ...
              React.createElement(AlertDescription, { key: 'desc' }, [
                ...
                React.createElement(
                  'ul',
                  { key: 'list', className: 'list-disc list-inside mt-2 space-y-1' },
                  [
                    ...
                    React.createElement(
                      'li',
                      { key: 'li3' },
                      'Upgrade to a premium plan for more storage'
                    ),
                  ]
                ),
              ]),
          ...
          React.createElement('div', { className: 'space-y-4 p-4' }, [
            React.createElement(Alert, { key: 'mobile', className: 'sm:hidden' }, [
              ...
              React.createElement(
                AlertDescription,
                { key: 'desc' },
                'This alert is optimized for mobile screens.'
              ),
            ]),
...
// Run validations immediately on module load to catch structural issues early
...
// --------------------------------------------
// Build indices once at module load for efficient access later.
...
Object.entries(scenarios).forEach(([componentName, group]) => {
  ...
  // Per-component scenario id -> scenario lookup
  ...
  list.forEach(scenario => {
    ...
    // Interactive scenarios (those with controls configured)
...
// Export cached results for reuse by callers that may want direct access
...
// Helper functions for accessing scenarios (use cached indices)
