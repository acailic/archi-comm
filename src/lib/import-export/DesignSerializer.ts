...
export class DesignSerializer {
  ...
  async importDesign(
    ...
    try {
      ...
      if (validation.requiresMigration) {
        parsedData = await this.migrateData(
          parsedData,
      ...
      if (options.mode === "merge") {
        ...
        if (conflicts.length > 0 && options.handleConflicts === "prompt") {
          ...
          result.warnings.push(
            `Found ${conflicts.length} conflicts that require user resolution`,
          );
          return result;
        }
      }
      ...
      if (options.mode === "merge" || !options.preserveIds) {
        processedDesign = this.resolveConflicts(
          processedDesign,
          result.conflicts,
          options,
        );
      ...
      result.statistics.connectionsImported =
        processedDesign.connections.length;
      result.statistics.infoCardsImported =
  ...
  private validateImportData(data: any): ImportValidationResult {
    ...
    if (!result.isCompatible) {
      if (this.canMigrate(data.formatVersion)) {
        ...
        result.warnings.push(
          `Format version ${data.formatVersion} requires migration`,
        );
      } else {
    ...
    if (
      data.metadata &&
      (!data.metadata.created || !data.metadata.lastModified)
    ) {
  ...
  private resolveConflicts(
    ...
    for (const conflict of conflicts) {
      if (
        options.handleConflicts === "auto" ||
        conflict.suggestedResolution === "rename"
      ) {
    ...
    resolvedDesign.connections = resolvedDesign.connections.map(
      (connection) => {
  ...
  private validateComponents(components: any[]): {
    ...
    for (const component of components) {
      ...
      if (typeof component.x !== "number" || typeof component.y !== "number") {
        warnings.push(
          `Component ${component.id}: invalid position coordinates`,
        );
      }
  ...
  static validateFileSize(
    file: File,
    maxSize: number = DEFAULT_VALIDATION_SCHEMA.maxFileSize,
  ): boolean {
  ...
  static validateFileExtension(
    file: File,
    allowedExtensions: string[] = DEFAULT_VALIDATION_SCHEMA.allowedExtensions,
  ): boolean {
