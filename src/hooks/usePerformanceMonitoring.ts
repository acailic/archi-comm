...
export function usePerformanceMonitoring(config: PerformanceConfig = {}) {
  ...
  const controls: PerformanceControls = useMemo(() => ({
    ...
    measureAsync: async <T>(name: string, operation: () => Promise<T>, metadata?: Record<string, any>) => {
    ...
    exportData: (): PerformanceExportData => {
      const exportData: PerformanceExportData = {
        ...
        customMetrics: Object.fromEntries(
          Array.from(customMetrics.current.entries())
        )
      };
  ...
  useEffect(() => {
    ...
    const checkMemoryPressure = () => {
      ...
      if (performanceData.memoryUsage > finalConfig.thresholds.maxMemoryUsage! * 0.8 && 
