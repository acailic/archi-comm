...
export const useUXTracker = () => {
   ...
   */
  const trackNavigation = useCallback((screen: string, previousScreen: string) => {
   ...
   */
  const trackCanvasAction = useCallback((actionType: string, data: Record<string, any>, success: boolean = true) => {
    const startTime = performance.now();
    ...
    try {
      ...
      if (duration > 100) { // Actions taking longer than 100ms
        addPerformanceError(`Slow canvas action: ${actionType}`, {
          renderTime: duration,
   ...
   * @param data - Action-specific data
   */
  const trackDialogAction = useCallback((actionType: string, dialogType: string, data: Record<string, any>) => {
    const startTime = performance.now();
   ...
   */
  const trackKeyboardShortcut = useCallback((shortcut: string, action: string, success: boolean = true) => {
    const startTime = performance.now();
  ...
  const trackPerformance = useCallback((metric: string, value: number, context?: Record<string, any>) => {
    ...
    try {
      ...
      if (threshold && value > threshold) {
        addPerformanceError(`Performance threshold exceeded for ${metric}`, {
  ...
  const trackError = useCallback((error: string | Error, context?: Record<string, any>) => {
    ...
    try {
      ...
      try {
        addError(trackingError instanceof Error ? trackingError : new Error(String(trackingError)), 'unknown', {
   ...
   */
  const getRecentUserActions = useCallback((limit: number = 5): string[] => {
    try {
   ...
   */
  const logUserInteractionPattern = useCallback((pattern: {
    sequence: string[];
    duration: number;
    success: boolean;
