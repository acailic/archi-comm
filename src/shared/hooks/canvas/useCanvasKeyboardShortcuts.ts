...
export interface CanvasKeyboardShortcutsConfig {
  ...
  onPenTool?: () => void;
  onEraserTool?: () => void;
  onHighlighterTool?: () => void;
  onExitDrawing?: () => void;
  onDecreaseStrokeSize?: () => void;
  onIncreaseStrokeSize?: () => void;
...
const shortcuts: KeyboardShortcut[] = [
  ...
  // Drawing tools
  { key: "d", description: "Activate drawing mode", action: "onDrawingMode" },
  { key: "p", description: "Select pen tool", action: "onPenTool" },
  { key: "e", description: "Select eraser tool", action: "onEraserTool" },
  {
    key: "h",
    description: "Select highlighter tool",
    action: "onHighlighterTool",
  },
  { key: "Escape", description: "Exit drawing mode", action: "onExitDrawing" },
  {
    key: "[",
    description: "Decrease stroke size",
    action: "onDecreaseStrokeSize",
  },
  {
    key: "]",
    description: "Increase stroke size",
    action: "onIncreaseStrokeSize",
  },
  {
    key: "d",
    ctrl: true,
    ...
    action: "onClearAllDrawings",
  },
  { key: "1", description: "Quick color: Black", action: "onQuickColor1" },
  { key: "2", description: "Quick color: Red", action: "onQuickColor2" },
  { key: "3", description: "Quick color: Blue", action: "onQuickColor3" },
  { key: "4", description: "Quick color: Green", action: "onQuickColor4" },
  { key: "5", description: "Quick color: Yellow", action: "onQuickColor5" },
  { key: "6", description: "Quick color: Purple", action: "onQuickColor6" },
  { key: "7", description: "Quick color: Orange", action: "onQuickColor7" },
  { key: "8", description: "Quick color: White", action: "onQuickColor8" },
  { key: "9", description: "Quick color: Custom", action: "onQuickColor9" },
...
export function useCanvasKeyboardShortcuts(
  config: CanvasKeyboardShortcutsConfig,
) {
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      const matchingShortcut = shortcuts.find((shortcut) => {
        ...
        const ctrlCorrect = shortcut.ctrl
...
export function getAllShortcuts(): Array<{
  ...
  const categorizedShortcuts = shortcuts.map((shortcut) => {
    ...
    let category = "Other";
    if (
      shortcut.action.includes("Tool") &&
      !shortcut.action.includes("Drawing")
    )
      category = "Annotation Tools";
    else if (
      shortcut.action.includes("Drawing") ||
      shortcut.action.includes("Pen") ||
      shortcut.action.includes("Eraser") ||
      shortcut.action.includes("Highlighter") ||
      shortcut.action.includes("Stroke") ||
      shortcut.action.includes("QuickColor")
    )
      ...
      category = "Editing";
    else if (["onSave", "onExport"].includes(shortcut.action))
      category = "File";
  ...
  const unique = categorizedShortcuts.reduce(
    (acc, curr) => {
