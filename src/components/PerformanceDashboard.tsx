...
const Chart: React.FC<ChartProps> = ({ 
  ...
  const points = data.map((d, i) => {
    ...
    const y = padding + chartHeight - ((d[metric] as number - minValue) / valueRange) * chartHeight;
    return `${x},${y}`;
  }).join(' ');
...
export function PerformanceDashboard({ isOpen, onClose, onToggleRecording }: PerformanceDashboardProps) {
  ...
  const createBaseline = () => {
    ...
    const avgRenderTime = performanceData.reduce((sum, d) => sum + d.renderTime, 0) / performanceData.length;
  ...
  const exportReport = () => {
    const report = {
      ...
      summary: {
        ...
        avgRenderTime: performanceData.reduce((sum, d) => sum + d.renderTime, 0) / performanceData.length,
        avgMemoryUsage: performanceData.reduce((sum, d) => sum + d.memoryUsage, 0) / performanceData.length,
  ...
  const analysis = useMemo(() => {
    ...
    return {
      ...
      bottlenecks,
      suggestions,
      health: avgFps > 50 && avgRenderTime < 16 && avgMemoryUsage < 70 ? 'good' : 
              avgFps > 30 && avgRenderTime < 25 && avgMemoryUsage < 85 ? 'warning' : 'critical'
    };
  }, [performanceData]);
            ...
            <div className="flex items-center space-x-3">
              ...
              {analysis && (
                <div className={`px-2 py-1 rounded text-xs font-medium ${
                  analysis.health === 'good' ? 'bg-green-100 text-green-800' :
                  analysis.health === 'warning' ? 'bg-yellow-100 text-yellow-800' :
              ...
              {analysis && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <PerformanceCard
                    ...
                    icon={Zap}
                    color={analysis.avgFps > 50 ? 'text-green-500' : analysis.avgFps > 30 ? 'text-yellow-500' : 'text-red-500'}
                  ...
                  <PerformanceCard
                    ...
                    unit="ms"
                    icon={Clock}
                    color={analysis.avgRenderTime < 10 ? 'text-green-500' : analysis.avgRenderTime < 20 ? 'text-yellow-500' : 'text-red-500'}
                  ...
                  <PerformanceCard
                    ...
                    unit="%"
                    icon={MemoryStick}
                    color={analysis.avgMemoryUsage < 60 ? 'text-green-500' : analysis.avgMemoryUsage < 80 ? 'text-yellow-500' : 'text-red-500'}
                          ...
                          {baselines.map((baseline, i) => (
                            ...
                            >
                              ...
                              <div className="text-muted-foreground">
                                FPS: {Math.round(baseline.avgFps)} | 
                                RT: {Math.round(baseline.avgRenderTime)}ms
                              </div>
                            </div>
                          ))}
                        </div>
