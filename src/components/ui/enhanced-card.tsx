...
import { animations, cx, designSystem, ElevationLevel, getElevation, getSize, getStyle, SizeVariant, StyleVariant } from '../../lib/design-system';
...
export const EnhancedCard = React.forwardRef<HTMLDivElement, EnhancedCardProps>(function EnhancedCard(
  {
    elevation = 1,
  ...
  const base = cx(
    ...
    interactive ? `${animations.hoverRaise} ${animations.pulseGlow} ${animations.focusRing} ${animations.tap}` : '',
    gradientBorder ? 'bg-clip-padding [background-clip:padding-box,border-box] border-transparent' : '',
    className
  );
        ...
        <div className="flex flex-col">
          {title && (
            <CardHeader className="pb-2">
              {typeof title === 'string' ? (
                <CardTitle className={cx('font-medium', designSystem.typography.md)}>{title}</CardTitle>
              ) : title}
            </CardHeader>
          )}
          <CardContent className={cx('relative', getSize(size))}>
            {children}
          </CardContent>
          {footer && (
