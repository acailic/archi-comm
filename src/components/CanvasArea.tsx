...
// Convert heavy imports to lazy imports
const PerformanceMonitor = React.lazy(() => import('../lib/performance/PerformanceOptimizer').then(m => ({ default: m.PerformanceMonitor })));
const MemoryOptimizer = React.lazy(() => import('../lib/performance/PerformanceOptimizer').then(m => ({ default: m.MemoryOptimizer })));
const OptimizedEventSystem = React.lazy(() => import('../lib/performance/PerformanceOptimizer').then(m => ({ default: m.OptimizedEventSystem })));

// Add lightweight loading state for canvas
...
export const CanvasArea = forwardRef<HTMLDivElement, CanvasAreaProps>(function CanvasArea({
  ...
  // Modify CanvasArea component to use lazy loaded performance utilities
      ...
      {connections.length > 0 && (
        <div className="absolute bottom-4 right-4 z-10">
          <div className="bg-card/80 backdrop-blur-xl border border-border/30 rounded-lg p-4 shadow-lg">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-3"><div className="w-6 h-0.5 bg-blue-500 rounded-full"></div><span className="text-xs text-muted-foreground">Data Flow</span></div>
              <div className="flex items-center space-x-3"><div className="w-6 h-0.5 bg-purple-500 rounded-full"></div><span className="text-xs text-muted-foreground">Control