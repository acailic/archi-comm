...
const LazyMotionDiv = React.lazy(() => import('framer-motion').then(m => ({ default: m.motion.div })));
const LazyAnimatePresence = React.lazy(() => import('framer-motion').then(m => ({ default: m.AnimatePresence })));
...
const useAnimations = (enabled: boolean = true) => {
  ...
  return { animationsReady, motionComponents, motion: motionComponents?.motion, AnimatePresence: motionComponents?.AnimatePresence };
};
...
export function SolutionHints({ 
  ...
  const hintsByType = useMemo(() => {
    return visibleHints.reduce((acc, hint) => {
      if (!acc[hint.type]) {
        acc[hint.type] = [];
      }
  ...
  const toggleHintExpansion = useMemo(() => (hintId: string) => {
  ...
  const containerProps = animationsReady && motion ? {
  ...
  return (
    ...
    >
      ...
      {animationsReady && motion ? (
        <Suspense fallback={<div className="p-4"><HintsLoadingSkeleton /></div>}>
          <motion.div
