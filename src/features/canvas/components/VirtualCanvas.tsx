...
const VirtualizationStatsOverlay: React.FC<{
  ...
  return (
    <Panel position="top-right" className="bg-card/95 backdrop-blur-sm border border-border/50 rounded-lg p-3 text-sm">
      <h4 className="font-semibold mb-2 text-foreground">Virtualization Stats</h4>
      <div className="space-y-1 text-muted-foreground">
        <div>Components: {stats.visibleComponents}/{stats.totalComponents}</div>
        <div>Connections: {stats.visibleConnections}/{stats.totalConnections}</div>
...
const VirtualCanvasInternal: React.FC<VirtualCanvasProps> = ({
  ...
  const { viewport, visibleBounds, isViewportStable } = useViewportTracking({
  ...
  const visibleNodes = useMemo((): Node[] => {
    const componentsToRender = config.enabled ? visibleComponents.map(item => item.data) : components;
  ...
  const visibleEdges = useMemo((): Edge[] => {
    const connectionsToRender = config.enabled ? visibleConnections.map(item => item.data) : connections;

    // Get visible node IDs to filter edges
    const visibleNodeIds = new Set(visibleNodes.map(node => node.id));

    // Filter connections to only include those with both endpoints visible
    const filteredConnections = connectionsToRender.filter(connection =>
  ...
  useEffect(() => {
    ...
    if (config.enablePerformanceMonitoring) {
      performanceManager.updateReactFlowCounts('virtual-canvas', visibleNodes.length, visibleEdges.length);
    }
  }, [
    config.enabled,
    config.debugMode,
      ...
      changes.forEach(change => {
        if (change.type === 'position' && change.position && change.dragging === false) {
          ...
          if (spatialIndex.isReady) {
            const component = components.find(c => c.id === change.id);
            if (component) {
  ...
  return (
    <div ref={canvasRef} className="w-full h-full relative">
      ...
      >
        ...
        {isLimited && (
          <Panel position="bottom-right" className="bg-amber-50 border border-amber-200 rounded-lg p-2 text-sm">
            <div className="text-amber-800">
              Performance mode: Showing {totalVisible}/{components.length + connections.length} items
            </div>
          </Panel>
        )}
      </ReactFlow>
