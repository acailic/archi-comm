...
export const CanvasAnnotationOverlay = forwardRef<
  CanvasAnnotationOverlayRef,
  CanvasAnnotationOverlayProps
>(
  (
    {
      width,
    ...
    const [editInputPosition, setEditInputPosition] = useState<{ x: number; y: number } | null>(
      null
    );
      ...
      if (canvasRef.current && !annotationManager.current) {
        ...
        return () => {
          ...
          canvas.removeEventListener(
            'annotationSelected',
            handleAnnotationSelected as EventListener
          );
          canvas.removeEventListener(
            'annotationEditStart',
            handleAnnotationEditStart as EventListener
          );
    ...
    const handleCanvasClick = useCallback(
      (event: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        performanceMonitor.current.measure('annotation-interaction', () => {
          ...
          try {
            const annotation = annotationManager.current!.addAnnotation(
              canvasX,
              canvasY,
              type,
              content,
