...
export function Minimap({
  ...
  const visibleConnections = useMemo(() => {
    ...
    return connections.filter(
      conn => visibleComponentIds.has(conn.from) && visibleComponentIds.has(conn.to)
    );
  }, [connections, visibleComponents]);

  const scale = useMemo(() => {
    // Guard against zero or invalid canvas extents
    if (!canvasExtents || canvasExtents.width <= 0 || canvasExtents.height <= 0) {
      return 1;
    }

    const availableWidth = MINIMAP_WIDTH - VIEWPORT_PADDING * 2;
    const availableHeight = MINIMAP_HEIGHT - VIEWPORT_PADDING * 2;
    ...
    const scaleX = availableWidth / canvasExtents.width;
    const scaleY = availableHeight / canvasExtents.height;
    const calculatedScale = Math.min(scaleX, scaleY, 1);

    // Ensure scale is a positive, finite number
    return calculatedScale > 0 && isFinite(calculatedScale) ? calculatedScale : 1;
  }, [canvasExtents]);
  ...
  const handleMinimapClick = useOptimizedCallback(
    (event: React.MouseEvent<SVGSVGElement>) => {
      if (!svgRef.current) return;

      // Guard against division by zero and invalid scale values
      if (!scale || scale <= 0 || !isFinite(scale)) {
        console.warn('Invalid scale value detected, skipping minimap click');
        return;
      }

      try {
        // Use SVG's built-in coordinate transformation methods
        const svg = svgRef.current;
        const ctm = svg.getScreenCTM();

        if (!ctm) {
          console.warn('Could not get screen CTM, falling back to manual calculation');
          // Fallback to manual calculation if CTM is not available
        ...
        // Transform screen coordinates to SVG space
        const svgPoint = point.matrixTransform(ctm.inverse());

        // Adjust for viewport padding and convert to canvas coordinates
        const x = svgPoint.x - VIEWPORT_PADDING;
        const y = svgPoint.y - VIEWPORT_PADDING;
        ...
        console.warn('SVG coordinate transformation failed, falling back to manual calculation:', error);
        // Fallback to manual calculation if SVG methods fail
