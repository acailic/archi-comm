...
describe('tauri.ts utility functions', () => {
  ...
  describe('diagramUtils', () => {
    it('saveDiagram invokes correct command', async () => {
      const { invoke } = await import('@tauri-apps/api/tauri');
      const mockElements = [
        { id: '1', element_type: 'box', position: { x: 0, y: 0 }, properties: {} },
      ];
      vi.mocked(invoke).mockResolvedValue(undefined);
    ...
    it('saveConnections invokes correct command', async () => {
      ...
      const mockConnections = [
        { id: '1', source_id: 'a', target_id: 'b', connection_type: 'line', properties: {} },
      ];
      vi.mocked(invoke).mockResolvedValue(undefined);
  ...
  describe('transcriptionUtils', () => {
    ...
    it('transcribeAudio validates parameters', async () => {
      ...
      await expect(
        transcriptionUtils.transcribeAudio('/test/audio.wav', { jobId: '' })
      ).rejects.toThrow('jobId must be a non-empty string');

      await expect(
        transcriptionUtils.transcribeAudio('/test/audio.wav', { maxSegments: -1 })
    ...
    it('ipcUtils.listen returns no-op function and warns', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'Tauri event listener "test_event" registered outside of Tauri environment'
        )
      );
