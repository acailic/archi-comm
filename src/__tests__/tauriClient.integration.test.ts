...
describe('tauriClient integration (contracts)', () => {
  it('createProject validates response against schema', async () => {
    (invoke as any).mockResolvedValue({
      id: 'p1', name: 'N', description: 'D', created_at: iso(), updated_at: iso(), status: 'Planning', components: [],
    });
  ...
  it('getProjects validates array', async () => {
    (invoke as any).mockResolvedValue([
      { id: 'p1', name: 'A', description: 'D', created_at: iso(), updated_at: iso(), status: 'Planning', components: [] },
      { id: 'p2', name: 'B', description: 'E', created_at: iso(), updated_at: iso(), status: 'Review', components: [] },
    ]);
